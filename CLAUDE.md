...
- Hero background image: `site/assets/hero.jpeg`（640px以下は`<picture>`で`hero-mobile.jpeg`に切替。元画像から生成した3:4の引き構図で、顔が右上・左下が暗い余白。`object-position: 78% 0%`）
- `hero-mobile.jpeg`はPillowで生成（右側1280pxを60%に縮小し、1280×1707の暗キャンバスへ右上配置・左端と下端をフェード合成）。元画像差し替え時は再生成する
