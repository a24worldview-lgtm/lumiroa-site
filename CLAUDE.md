...
- `site/index.html`: サイトのHTML本体。ヒーロー、コンセプト、施術、FLOW、MENU、ギャラリー、アクセス、CTA、フッターを管理。
- `site/styles.css`: デザイントークン、レイアウト、レスポンシブ、ボタン、写真帯、セクションごとの見た目を管理。
- `site/script.js`: Hot Pepper Beauty URLの一括反映、スマホメニュー開閉、ヘッダーのスクロール状態、reveal演出を管理。
...
- FLOWの空ボックス: 画面幅や将来項目数の変更でカードに余白や空き感が出る場合があるため、最終表示でカード数・高さ・行揃えを再確認する。
- 見出しの改行制御: Heroは固定2行。その他の長い日本語見出しは端末幅によって改行位置が変わるため、公開前に主要端末で確認する。
- ヘッダーの視認性: Hero写真上では白ロゴ・白ナビ、スクロール後は淡い背景。写真差し替え時はコントラストを再確認する。
- 写真帯の整理: Galleryは3枚構成。実店舗写真が増えたら、人物写真と商品写真の比率を見直す。
